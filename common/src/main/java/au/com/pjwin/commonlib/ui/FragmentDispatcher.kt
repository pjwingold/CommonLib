...
class FragmentDispatcher(host: BaseActivity<*, *, *>) : LifecycleObserver {
    private var hostActivity: BaseActivity<*, *, *>? = host
    private var lifeCycle: Lifecycle? = host.getLifecycle()
    ...
    private fun replaceFragment(@IdRes container: Int, fragment: Fragment, animate: Boolean) {
        hostActivity?.let {
            val fragmentTransaction = it.getSupportFragmentManager().beginTransaction()
